...
namespace Microsoft.Cci.Ast {
  ...
  /// <summary>
  /// A region comprehension.
  /// </summary>
  public class RegionUnion : Quantifier //TODO: perhaps this is too methodology specific to live here
  {
    /// <summary>
    /// Allocates a region comprehension. 
    /// </summary>
    /// <param name="boundVariables">
    /// One of more local declarations statements. Typically these locals are referenced in this.Condition
    /// and in this.RegionExpression.
    /// </param>
    /// <param name="condition">
    /// An expression that is evaluated for every tuple of values that may be bound to the variables defined by this.BoundVariables.
    /// Typically, the expression contains references to the variables defined in this.BoundVariables.
    /// </param>
    /// <param name="regionExpr">
    /// The result of a RegionUnion is a union of regions resulting from computing this expression
    /// for every tuple of values that may be bound to the variables defined by this.BoundVariables
    /// and for which this.Condition is true.
    /// Typically, the expression contains references to the variables defined in this.BoundVariables.
    /// </param>
    /// <param name="sourceLocation">The source location corresponding to the newly allocated expression.</param>
    public RegionUnion(List<LocalDeclarationsStatement> boundVariables, Expression condition, Expression regionExpr, ISourceLocation sourceLocation)
      : base(boundVariables, condition, sourceLocation) {
    ...
    /// <param name="containingBlock">A new value for containing block. This replaces template.ContainingBlock in the resulting copy of template.</param>
    /// <param name="template">The template to copy.</param>
    protected RegionUnion(BlockStatement containingBlock, RegionUnion template)
      : base(containingBlock, template)
      //^ requires template.ContainingBlock != containingBlock;
      //^ ensures this.containingBlock == containingBlock;
    {
      this.regionExpr = template.regionExpr;
    }
    ...
    {
      if (containingBlock == this.ContainingBlock) return this;
      RegionUnion result = new RegionUnion(containingBlock, this);
      result.CopyTriggersFromTemplate(this);
      return result;
    }
    ...
    public override Expression Condition {
      get {
        ...
        Expression inRegionRef = NamespaceHelper.CreateInSystemDiagnosticsContractsCodeContractExpr(this.NameTable, "InRegion");
        Expression[] args = new Expression[] { base.Condition, this.regionExpr };
        cachedCondition = new MethodCall(inRegionRef, args, SourceDummy.SourceLocation);
