...
namespace VBSourceEmitter {
  public partial class SourceEmitter : CodeTraverser, IVBSourceEmitter {
    public override void TraverseChildren(IMethodDefinition methodDefinition) {
      ...
      var voidType = TypeHelper.TypesAreEquivalent(methodDefinition.Type, this.host.PlatformType.SystemVoid);
      if (voidType) {
        PrintToken(VBToken.Sub);
      } else {
        PrintToken(VBToken.Function);
      }
      ...
      if (!methodDefinition.IsAbstract && !methodDefinition.IsExternal) {
        ...
        PrintToken(VBToken.Space);
        PrintToken(voidType ? VBToken.Sub : VBToken.Function);
      } else {
    ...
    public virtual void PrintMethodDefinitionName(IMethodDefinition methodDefinition) {
      ...
      if (methodDefinition.IsConstructor)
        ...
        sourceEmitterOutput.Write(MapOperatorNameToVB(methodDefinition));
      } else
        PrintIdentifier(methodDefinition.Name);
    }

    public virtual string MapOperatorNameToVB(IMethodDefinition methodDefinition) {
    ...
    public virtual void PrintMethodReferenceName(IMethodReference methodReference, NameFormattingOptions options) {
      string signature = this.helper.GetMethodSignature(methodReference, options);
      //string signature = MemberHelper.GetMethodSignature(methodReference, options | NameFormattingOptions.ContractNullable | NameFormattingOptions.UseTypeKeywords | NameFormattingOptions.OmitTypeArguments);
